...
export function SettingsView({ 
  ...
  return (
    <div className="flex-1 p-3.5 sm:p-6 lg:p-8 pb-[calc(4.5rem+env(safe-area-inset-bottom,0px))] md:pb-8 space-y-4 sm:space-y-8 overflow-y-auto h-full bg-[#F4F6F9] dark:bg-[#0a0a0f]">
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 bg-white dark:bg-[#0d1117] rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-[#00D4B2]/10 relative overflow-hidden">
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#0055FF]/10 dark:bg-[#0055FF]/15 text-[#0055FF] dark:text-[#6699ff] border border-[#0055FF]/20 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-2">
            System Preferences
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Preferences & Settings</h1>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage system theme choices, scheme integrations, and user preferences.</p>
        </div>
      </div>

      {/* Account Profile Summary Card */}
      <div className="bg-white dark:bg-[#0d1117] rounded-2xl sm:rounded-3xl p-4 sm:p-6 border border-gray-100 dark:border-white/5 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-3 sm:gap-4">
          <div className="w-12 sm:w-16 h-12 sm:h-16 shrink-0 rounded-xl sm:rounded-2xl bg-[#00D4B2]/10 dark:bg-white/5 border border-[#00D4B2]/30 dark:border-white/10 text-[#00D4B2] font-black text-base sm:text-xl flex items-center justify-center select-none shadow-sm">
            {activePersonaName ? activePersonaName.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase() : 'U'}
          </div>
          <div>
            <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white leading-tight">{activePersonaName}</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold mt-0.5 sm:mt-1">{activePersonaRole}</p>
            <div className="inline-flex items-center gap-1 text-[9px] sm:text-[10px] bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider mt-1.5 sm:mt-2 border border-emerald-500/20">
      ...
      <div className="bg-white dark:bg-[#0d1117] rounded-3xl p-6 border border-gray-100 dark:border-white/5 shadow-sm space-y-6">
        ...
        <form onSubmit={handleSaveProfile} className="space-y-5">
          ...
          <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-2.5 sm:gap-0 pt-2">
            <button
              type="button"
              onClick={() => {
              ...
              className="px-4 py-3 sm:py-2.5 rounded-2xl border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-400 font-semibold text-xs hover:bg-gray-100 dark:hover:bg-white/5 transition-all cursor-pointer min-h-[44px] active:scale-95 flex items-center justify-center"
            >
              Discard Changes
            </button>
            <button
              type="submit"
              disabled={isSaving}
              className="px-6 py-3 rounded-2xl bg-gradient-to-r from-[#00D4B2] to-[#0055FF] text-white font-bold text-xs uppercase tracking-wider shadow-md hover:opacity-95 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 min-h-[44px] active:scale-95"
            >
              {isSaving ? (
      ...
      <div className="space-y-3 sm:space-y-4">
        <div>
          <h3 className="text-sm sm:text-base font-bold text-gray-900 dark:text-white tracking-tight uppercase">Display Theme Preference</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6">
          ...
          <div 
            ...
            className={`group p-4 sm:p-5 rounded-2xl sm:rounded-[28px] border transition-all cursor-pointer relative overflow-hidden flex flex-col justify-between min-h-[140px] sm:min-h-[160px] select-none active:scale-[0.98] ${
              theme === 'light' 
      ...
      <div className="bg-gray-50 dark:bg-[#0d1117]/50 p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-gray-200/60 dark:border-gray-800 space-y-2.5 sm:space-y-3">
