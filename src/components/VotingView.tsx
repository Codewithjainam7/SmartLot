    ...
    <div className="flex-1 p-8 space-y-8 overflow-y-auto h-full bg-[#F4F6F9] dark:bg-[#0a0a0f]">
      ...
      <div className="space-y-6">
        {motions.map(motion => {
          ...
          return (
            <div key={motion.id} className="bg-white dark:bg-[#0d1117] rounded-3xl p-8 border border-gray-100 dark:border-white/5 shadow-sm space-y-6 relative overflow-hidden">
              ...
              {/* Problem Summary */}
              <p className="text-sm text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-[#1a1d27] p-4 rounded-2xl border border-gray-100 dark:border-white/5">
              ...
              <div>
                <h4 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Attached Contractor Quotes (Comparison)</h4>
                  ...
                  {motion.quotes.map(quote => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-bold text-gray-900 dark:text-white">{quote.vendorName}</span>
                        {quote.recommended && (
              ...
              {/* Live Quorum Meter */}
              <div className="bg-[#F4F6F9] dark:bg-[#0a0a0f] p-6 rounded-2xl space-y-3">
                <div className="flex items-center justify-between text-sm font-bold">
                  ...
                  <span className="text-gray-600 dark:text-gray-300">{totalVotes} / {motion.quorumTarget} Committee Votes ({quorumProgress}%)</span>
                </div>

                <div className="w-full bg-gray-200 dark:bg-white/5 rounded-full h-3 overflow-hidden">
                  <div 
              ...
              {motion.status === 'active' && (
                <div className="pt-4 border-t border-gray-100 dark:border-white/5">
                  {hasVoted ? (
                    <div className="bg-[#0055FF]/10 border border-blue-100 dark:border-blue-900/30 p-4 rounded-2xl text-center text-sm font-bold text-blue-900 dark:text-blue-400 flex items-center justify-center gap-2">
                      <CheckCircle2 size={18} className="text-[#00D4B2]" /> Your Digital Ballot has been securely logged for this motion.
                    </div>
                  ) : (
                    <div>
              ...
              {motion.status === 'passed' && (
                <div className="bg-[#00D4B2]/10 border border-[#00D4B2]/30 p-5 rounded-2xl flex items-center justify-between text-emerald-900 dark:text-emerald-300">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-emerald-700 dark:text-emerald-400">Digital Work Order <span className="font-bold">{motion.createdWorkOrderId}</span> was auto-generated and dispatched to contractor.</div>
                    </div>
                  </div>
                </div>
              )}
