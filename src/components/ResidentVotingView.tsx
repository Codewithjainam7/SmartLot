...
export function ResidentVotingView({
  ...
  return (
    <div className="flex-1 p-8 space-y-8 overflow-y-auto h-full bg-[#F4F6F9]">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-[#121316] to-[#1E2026] text-white rounded-3xl p-8 shadow-md">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#8B8CF8]/20 text-[#8B8CF8] text-xs font-bold uppercase tracking-wider mb-2">
              ...
              <button
                ...
                className={`w-full text-left p-5 rounded-2xl border transition-all cursor-pointer ${
                  activeDetail?.id === req.id 
                    ? 'bg-[#121316] text-white border-black shadow-md' 
                    : 'bg-gray-50 border-gray-100 hover:bg-gray-100 text-gray-900'
                }`}
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-xs font-bold uppercase ${activeDetail?.id === req.id ? 'text-[#8B8CF8]' : 'text-gray-400'}`}>{req.id} • {req.unit}</span>
                  <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded ${activeDetail?.id === req.id ? 'bg-[#D8F235] text-[#121316]' : 'bg-purple-100 text-[#8B8CF8]'}`}>
                    IN VOTING
                  </span>
                </div>
        ...
        {activeDetail && (
          <div className="lg:col-span-7 bg-white rounded-3xl p-8 border border-gray-100 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-gray-100 pb-4">
              <div>
                <span className="text-xs font-bold text-[#8B8CF8] uppercase tracking-wider">{activeDetail.id} • {activeDetail.unit}</span>
                <h2 className="text-2xl font-bold text-gray-900">{activeDetail.title}</h2>
              </div>
              <span className="px-3 py-1 rounded-full bg-purple-100 text-[#8B8CF8] text-xs font-bold uppercase">
                IN VOTING
              </span>
            </div>
            ...
            <div className="bg-[#F4F6F9] p-6 rounded-2xl space-y-4">
              <div className="flex items-center justify-between text-sm font-bold">
                <span className="text-gray-900 flex items-center gap-2">
                  <Vote size={18} className="text-[#8B8CF8]" /> Voting Tally
                </span>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs font-bold">
                ...
                <div className="bg-red-50 border border-red-200 p-3 rounded-xl flex items-center justify-between text-red-900">
                  <span>NO Votes:</span>
              ...
              {activeDetail.userVoted && (
                <div className="bg-blue-50 border border-blue-200 p-3 rounded-xl text-center text-xs font-bold text-blue-900 flex items-center justify-center gap-1.5">
                  <CheckCircle2 size={16} className="text-blue-600" /> Your vote has been logged for this request.
                </div>
              )}
            </div>
            ...
            <div className="space-y-4 pt-4 border-t border-gray-100">
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2">
                <MessageSquare size={16} className="text-[#8B8CF8]" /> Discussion & Comments ({activeDetail.comments.length})
              </h4>
              ...
              <form onSubmit={handleSendComment} className="flex gap-2 pt-2">
                ...
                />
                <button type="submit" className="bg-[#121316] text-white px-5 py-3 rounded-2xl text-xs font-bold flex items-center gap-1.5 cursor-pointer">
                  <Send size={14} /> Post
                </button>
              </form>
            </div>
