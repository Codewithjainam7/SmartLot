...
export function UserManagementView({
  ...
  const [isRosterRoleDropdownOpen, setIsRosterRoleDropdownOpen] = useState(false);
  const rosterRoleDropdownRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (rosterRoleDropdownRef.current && !rosterRoleDropdownRef.current.contains(e.target as Node)) {
        setIsRosterRoleDropdownOpen(false);
      }
    };
    if (isRoleDropdownOpen || isRosterRoleDropdownOpen) {
      document.addEventListener('mousedown', handleClickOutside);
    }
    return () => {
      document.removeEventListener('mousedown', handleClickOutside);
    };
  }, [isRoleDropdownOpen, isRosterRoleDropdownOpen]);
  ...
  const roleFilterOptions = useMemo(() => [
    ...
    { label: 'Managers', value: 'Strata Manager', count: members.filter(m => m.role === 'Strata Manager' || m.role === 'Building Manager' || (m.role as any) === 'Strata Admin').length },
    ...
    { label: 'Service Providers', value: 'Service Provider', count: members.filter(m => (m.role as any) === 'Service Provider').length },
  ], [members]);

  const activeRoleOption = useMemo(() => {
    if (colFilterRole === 'Strata Manager' || colFilterRole === 'Managers' || colFilterRole === 'Building Manager') {
      return roleFilterOptions.find(opt => opt.value === 'Strata Manager') || roleFilterOptions[0];
    }
    return roleFilterOptions.find(opt => opt.value === colFilterRole) || roleFilterOptions[0];
  }, [colFilterRole, roleFilterOptions]);
        ...
        {activeTab === 'roster' ? (
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between gap-2.5">
              <div ref={rosterRoleDropdownRef} className="relative z-30">
                <button
                  type="button"
                  onClick={() => setIsRosterRoleDropdownOpen(prev => !prev)}
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <div className={`w-7 h-7 rounded-lg bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-500 dark:text-gray-400 transition-transform duration-200 group-hover:text-gray-900 dark:group-hover:text-white ${isRosterRoleDropdownOpen ? 'rotate-180 text-[#0055FF] dark:text-[#00D4B2]' : ''}`}>
                      <ChevronDown size={14} />
                    </div>
                  </div>
                </button>
                    ...
                    >
                      <div className="p-1.5 space-y-0.5">
                        {roleFilterOptions.map(opt => {
                          const isSelected = activeRoleOption.value === opt.value;
                          return (
                            <button
                              key={opt.value}
                              type="button"
                              onClick={() => {
                                setColFilterRole(opt.value);
                                setIsRosterRoleDropdownOpen(false);
                              }}
              ...
              {activeFiltersCount > 0 && (
                <button
                  ...
                  className="px-3 py-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer border border-red-500/20 shadow-xs"
                >
...
function AddMemberFormContent({ 
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    const primaryToken = `INV-${crypto.randomUUID().replace(/-/g, '').substring(0, 12).toUpperCase()}`;
    ...
    try {
      ...
      for (const occ of validOccupants) {
        const occToken = `INV-${crypto.randomUUID().replace(/-/g, '').substring(0, 12).toUpperCase()}`;
